...
export default function RakelOperationsPage() {
  ...
  const rows = useMemo((): CompanyOpsRow[] =>
    companies.map(company => {
      const recs   = records.filter(r => r.companyId === company.id);
      const avgPerf = recs.length > 0
        ? Math.round(recs.reduce((s, r) => s + r.performanceScore, 0) / recs.length) : 0;
      const dates  = recs.map(r => r.date).sort();
      return { company, totalEntries: recs.length, avgPerformance: avgPerf, latestDate: dates[dates.length - 1] ?? null };
    }), [companies, records]);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Select a company to view and log operational data.</p>
      </div>
                ...
                <tbody>
                  {rows.map(({ company, totalEntries, avgPerformance, latestDate }) => (
                    <tr
                      ...
                      onClick={() => router.push(`/rakel/operations/${company.id}`)}
                    >
                      ...
                      <td className="px-4 py-3 text-foreground">
                        {totalEntries > 0 ? <span className="font-semibold">{totalEntries}</span> : <span className="text-muted-foreground">—</span>}
                      </td>
                      <td className="px-4 py-3">
                        {totalEntries > 0 ? (
                          <span className={`font-semibold ${avgPerformance >= 80 ? 'text-primary' : avgPerformance >= 60 ? 'text-chart-3' : 'text-destructive'}`}>
                            {avgPerformance}%
                          </span>
                        ) : <span className="text-muted-foreground">—</span>}
                      </td>
                      <td className="px-4 py-3 text-muted-foreground whitespace-nowrap">
                        {latestDate ? new Date(latestDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }) : '—'}
                      </td>
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${company.isActive !== false ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'}`}>
                      ...
                      <td className="px-4 py-3">
                        <Button
                          ...
                          onClick={e => { e.stopPropagation(); router.push(`/rakel/operations/${company.id}`); }}
                        >

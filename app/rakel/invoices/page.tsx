...
export default function RakelInvoicesPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Select a company to view and manage its invoices.</p>
      </div>
                ...
                <tbody>
                  {rows.map(({ company, total, paid, pending, overdue, totalAmt }) => (
                    <tr key={company.id}
                      ...
                      onClick={() => router.push(`/rakel/invoices/${company.id}`)}>
                      ...
                      <td className="px-4 py-3"><span className={`font-semibold ${paid > 0 ? 'text-primary' : 'text-muted-foreground'}`}>{paid}</span></td>
                      <td className="px-4 py-3"><span className={`font-semibold ${pending > 0 ? 'text-chart-3' : 'text-muted-foreground'}`}>{pending}</span></td>
                      <td className="px-4 py-3"><span className={`font-semibold ${overdue > 0 ? 'text-destructive' : 'text-muted-foreground'}`}>{overdue}</span></td>
                      ...
                      <td className="px-4 py-3">
                        <Button size="sm" variant="outline" className="border-border hover:border-primary/50 gap-1"
                          onClick={e => { e.stopPropagation(); router.push(`/rakel/invoices/${company.id}`); }}>

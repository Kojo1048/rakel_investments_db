...
export default function CEOCompanyDocumentsDetailPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return documents.filter(doc => {
      ...
      const matchTime = !cutoff || new Date(doc.uploadedAt) >= cutoff;
      return matchSearch && matchType && matchCat && matchTime;
    });
  }, [documents, searchTerm, fileTypeFilter, categoryFilter, timeFilter]);

  if (!mounted) return null;

  const clearFilters = () => { setSearchTerm(''); setFileTypeFilter('all'); setCategoryFilter('all'); setTimeFilter('all'); };
  const hasFilters = searchTerm || fileTypeFilter !== 'all' || categoryFilter !== 'all' || timeFilter !== 'all';

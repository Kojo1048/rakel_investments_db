...
import { Building2, ChevronRight, Activity, Search, AlertCircle } from 'lucide-react';
...
export default function SuperAdminOperationsPage() {
  ...
  useEffect(() => {
    ...
    // Force-resolve after 8s so the page never stays stuck
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-foreground">Operations</h1>
        <p className="text-muted-foreground">Select a company to view its operational records.</p>
      </div>
            ...
            <Card key={company.id} className="bg-card border-border hover:border-primary/50 transition-colors">
              <CardContent className="p-5">
                ...
                <Button
                  ...
                  onClick={() => router.push(`/admin/operations/${company.id}`)}
                >
                  <span className="flex items-center gap-2">
                    <Activity className="h-4 w-4" />View Operations
                  </span>
